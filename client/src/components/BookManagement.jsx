...
const BookManagement = () => {
  ...
  const { loading, books, error, message } = useSelector((state) => state.book);

  // Fix: Get user from auth state instead of user state
  ...
  const searchedBooks = books.filter(
    (book) => book.title.toLowerCase().includes(searchedKeyword.toLowerCase())
    ...
    <>
      <main className="relative flex-1 p-6 pt-10">
        ...
        <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
          ...
          <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
            ...
            <input
              ...
              value={searchedKeyword} // Changed from searchedBooks to searchedKeyword
        ...
        {books && books.length > 0 ? (
          ...
          <h3 className="text-3xl">No books found in library!</h3>
        )}
      </main>
