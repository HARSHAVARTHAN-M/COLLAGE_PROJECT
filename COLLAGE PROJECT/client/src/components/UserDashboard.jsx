...
const UserDashboard = () => {
  ...
  useEffect(() => {
    if (userBorrowedBooks && Array.isArray(userBorrowedBooks)) {
      console.log("UserBorrowedBooks:", userBorrowedBooks); // Add this to debug
      let numberofTotalBorrowedBooks = userBorrowedBooks.filter(
        (book) => !book.returned && book.returnDate === null
      );
      let numberofTotalReturnedBooks = userBorrowedBooks.filter(
        (book) => book.returned && book.returnDate !== null
      );
      console.log("Borrowed:", numberofTotalBorrowedBooks.length);
      console.log("Returned:", numberofTotalReturnedBooks.length);
  ...
  return (
    <>
      <main className="relative flex-1 p-6 pt-10">
        <Header />
            ...
            <div className="flex items-center p-8 w-full sm:w-[400px] xl:w-fit mr-5 xl:p-3 2xl:p-6 gap-5 h-fit xl:min-h-[150px] bg-white xl:flex-1 rounded-lg">
              ...
              <div className="flex flex-col gap-5">
                ...
                <p className="flex items-center gap-3">
                  ...
                  <span>Total Returned Books : {totalReturnBooks}</span>
                </p>
              </div>
            </div>
          </div>
